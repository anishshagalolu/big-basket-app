...
function Milk()
{
    ...
    let dispatch=useDispatch()
     ...
     const[ currentPage, setCurrentPage ] = useState(1);
        const itemsPerPage = 5; 
           ...
           milkproducts:milkproducts.filter(product=>
    ...
    let milkListItems=paginatedItems.map((product,index)=>(
        
        <li key={index}>
            <img src={product.image} alt={product.name}/>
            <h3>{product.name}</h3><p>{product.price} Rs</p>
            
            <button onClick={()=>{dispatch(AddToCart(product)); toast.success(`${product.name} Added to Cart`)}}>Add to Cart</button>
        </li>
    ))
    return(
